以下是补充了 `pi-hermes-memory` 后的完整 `readme.md`。该插件新增为 **第 6 章“持久记忆”** ，后续章节编号顺延。

---

# Pi 插件安装与配置清单

请安装并配置以下 Pi 插件，所有插件均需在 `settings.json` 中配置为按需加载（Lazy Load）。

> **重要提示**：Pi 的 Lazy Load 并非通过 `settings.json` 的某个全局开关直接生效，而是需要安装专门的延迟加载管理器。以下配置流程默认以社区最常用的 **`@rahularya01/pi-lazy`** 为基础。部分插件（如 `pi-tool-search`）自身具备按需加载机制，无需额外配置。

## 0. 延迟加载管理器（前置安装）

- `pi install npm:@rahularya01/pi-lazy`
- **功能**：提供 LazyVim 风格的扩展管理能力。保持所有包处于已安装状态，但仅在首次使用（通过斜杠命令、工具调用或关键词触发）时才执行其扩展工厂，从而加快 Pi 启动速度并缩短模型需要推理的工具列表。
- **首次配置（必须）** ：安装后重启 Pi，运行 `/lazy migrate`，然后再次重启。该命令会备份 `~/.pi/agent/settings.json`，并将所有包从字符串形式重写为对象形式（如 `{ "source": "npm:pi-web-access", "extensions": [] }`），阻止 Pi 在启动时 eager-load 这些包的扩展。
